...
object InterstitialAdManager {
    ...
    fun preload(context: Context) {
        ...
        val autoEnabled = config.interstitial.enabled &&
            AutoInterstitialManager.isEnabledForCurrentSession() &&
    ...
    fun onEligibleTransition(
        ...
        val interstitialCount = session.count(AdPlacement.INTERSTITIAL) +
            session.count(AdPlacement.AUTO_INTERSTITIAL)
        val intervalElapsed = System.currentTimeMillis() - session.lastInterstitialShownAt >=
            config.interstitial.minIntervalSeconds * 1000L
        val sharedInterstitialAllowed = config.interstitial.enabled &&
            interstitialCount < config.interstitial.maxPerSession && intervalElapsed
        if (!activitySafe) {
        ...
        val autoEligible = if (autoTypeAllowed) {
            AutoInterstitialManager.onEligibleEvent(
                ...
                sessionAllowed = sessionAllowed && sharedInterstitialAllowed
            )
        } else {
        ...
        val canShow = eligibleCount % placement.frequency == 0 && intervalElapsed &&
            interstitialCount < placement.maxPerSession
        val loaded = ad
    ...
    private fun showAutoInterstitialOrProceed(
        ...
        val blockedReason = when {
            ...
            !config.interstitial.enabled -> "interstitial placement disabled"
            !AdSessionManager.canShowAd(config) -> "session/global cap reached"
            AdSessionManager.snapshot.value.let { session ->
                session.count(AdPlacement.INTERSTITIAL) +
                    session.count(AdPlacement.AUTO_INTERSTITIAL)
            } >= config.interstitial.maxPerSession -> "interstitial session cap reached"
            System.currentTimeMillis() - AdSessionManager.snapshot.value.lastInterstitialShownAt <
                config.interstitial.minIntervalSeconds * 1000L -> "interstitial minimum interval"
    ...
    fun onOnboardingCompleted(activity: Activity, activitySafe: Boolean): Boolean {
        ...
        if (!activitySafe || !config.masterEnabled || !placement.enabled || !typeAllowed ||
            !AdRuntime.canLoadAds() || !AdSessionManager.canShowAd(config) ||
